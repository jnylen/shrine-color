module ShrineColor
  module Metrics
    ...
    def self.delta_e_cie_2000(l1, a1, b1, l2, a2, b2)
      ...
      t =
        1 - 0.17 * Math.cos(radians(avg_hp - 30)) +
          0.24 * Math.cos(radians(2 * avg_hp)) +
      ...
      delta_e =
        Math.sqrt(
          ((delta_lp / (s_l * kl))**2) + ((delta_cp / (s_c * kc))**2) +
            ((delta_hp / (s_h * kh))**2) +
    ...
    def self.scale(number, from_range, to_range = 0..1, clamp = true)
      if clamp && number <= from_range.begin
        ...
        position =
          (number - from_range.begin).to_f / (from_range.end - from_range.begin)
      end
